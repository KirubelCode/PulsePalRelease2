...
public class SendLogsToGoogleDriveActivity extends AppCompatActivity implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener,
    ...
    private void queryLogsFile() {
        ...
        File dirFile = getApplicationContext().getExternalFilesDir(null);
        if (dirFile.exists()) {
    ...
    private void chooseAccount() {
        ...
        if (EasyPermissions.hasPermissions(
            ...
            // Try to set account name if in prefs
            if (accountName != null) {
                mCredential.setSelectedAccountName(accountName);
            }
            if (mCredential.getSelectedAccountName() != null) {
                // Setting account name was succesful, do the rest
                getResultsFromApi();
            }
            else {
            ...
            if (mLastError != null) {
                if (mLastError instanceof GooglePlayServicesAvailabilityIOException) {
                    ...
                    Toast.makeText(SendLogsToGoogleDriveActivity.this, "The following error occurred:\n"
                            + mLastError.getClass().getSimpleName(), Toast.LENGTH_LONG).show();
                    Log.e(LOG_TAG, "Error sending file to GoogleDrive:", mLastError);
                }
            } else {
